...
# --- EXPORTACIÓN ---
cat(sprintf("\n💾 Exportando con keyword '%s'...\n", keyword))
gl2wildcard(gl10, paste0(keyword, ".wdc"))
...
gl2vcf(gl10,       outfile = basename(keyword),                        outpath = savepath, plink.bin.path = "./plink")
gl2fasta(gl10,     outfile = paste0(basename(keyword), ".fasta"),      outpath = savepath, method = 3)
gl2plink(gl10,     outfile = basename(keyword),                        outpath = savepath, plink.bin.path = "./plink", bed.files = TRUE)
