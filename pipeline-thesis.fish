...
# INPUTS
# ─────────────────────────────────────────────
set RAW_DART   $WORKDIR"/data/Report_DErios22-7421_SNP_2.csv"
set COV_FILE   $WORKDIR"/data/Eriosyce_bufferpops.csv"
set ENV_FILE   $WORKDIR"/data/envar_final-corto.csv"
...
$RAPP $WORKDIR/selective_loci_lfmm-gf.r \
    ...
    --gl_rds         $GL_RDS \
    --outfile_rds        $WORKDIR"/step5_obj/"$PFX"_loci_clasificados.rds" \
...
for CAT in true_neutral selected_and selected_or putative_local
    set STR_CAT  $WORKDIR"/step5_textfiles/strfile_"$CAT".str"
    sed -i '1s/^/id\t/' $STR_CAT
    $RAPP $WORKDIR/format_ba3.r \
        --str_file       $STR_CAT\
        --poptable_file $COV_FILE \
        --outfile $WORKDIR"/step5_textfiles/"$CAT"_ba3input.txt"
    or exit 1
end
# ─────────────────────────────────────────────
# STEP 6 — IQ-TREE
...
for CAT in true_neutral selected_and selected_or putative_local
    BA3 -F $WORKDIR"/step5_textfiles/"$CAT"_ba3input.txt" \
